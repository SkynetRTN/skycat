...
class SourceManifest(CatalogBase):
    """A snapshot of the discovered source files backing a release."""
...
class SourceFile(CatalogBase):
    """An individual file recorded in a :class:`SourceManifest`."""
    ...
    manifest_id: Mapped[int] = mapped_column(
        ForeignKey(f"{SCHEMA_REGISTRY}.source_manifest.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    manifest: Mapped[SourceManifest] = relationship(back_populates="files")
