    ...
    try:
        with engine.connect() as conn:
            rows = conn.execute(
                ...
                {"s": [SCHEMA_DATA, SCHEMA_REGISTRY, SCHEMA_STAGING]},
            ).mappings()
...
def revalidate_release(
    settings: CatalogSettings, family_slug: str, release_ref: str
) -> dict:
    ...
    try:
        with Session(engine) as session:
            ...
            if release is None or not release.production_table:
                raise CatalogConfigError(
                    f"No imported release {family_slug}/{release_ref}"
                )
        ...
        checks.append(
            Check("production_ra_range", CRITICAL, bad_ra == 0, f"{bad_ra} bad RA")
        )
        checks.append(
            Check("production_dec_range", CRITICAL, bad_dec == 0, f"{bad_dec} bad Dec")
        )
