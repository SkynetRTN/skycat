...
from skynet_catalogs.config import (
    CatalogConfigError,
...
def test_url_construction_and_redaction():
    cfg = CatalogDatabaseConfig(
        backend="postgresql+psycopg", host="catalog-postgres", port=5432,
        name="catalogs", user="catalog_reader", password="p@ss/w:rd",
    )
    url = cfg.url()
    assert url.startswith("postgresql+psycopg://catalog_reader:")
    assert "@catalog-postgres:5432/catalogs" in url
...
def test_docker_internal_vs_host_configs():
    internal = CatalogDatabaseConfig(host="catalog-postgres", port=5432)
...
def test_production_like_detection():
    ...
    assert not CatalogDatabaseConfig(host="127.0.0.1", name="catalogs").looks_production()


def test_role_resolution_from_env(monkeypatch):
    monkeypatch.setenv("SKYNET_CATALOG_DB_HOST", "catalog-postgres")
    monkeypatch.setenv("SKYNET_CATALOG_DB_PORT", "5432")
    monkeypatch.setenv("SKYNET_CATALOG_DB_USER", "catalog_reader")
    monkeypatch.setenv("SKYNET_CATALOG_DB_PASSWORD", "rpw")
    monkeypatch.setenv("SKYNET_CATALOG_DB_ADMIN_USER", "catalog_owner")
    monkeypatch.setenv("SKYNET_CATALOG_DB_ADMIN_PASSWORD", "opw")
    monkeypatch.setenv("SKYNET_CATALOG_DB_INGEST_USER", "catalog_ingest")
    monkeypatch.setenv("SKYNET_CATALOG_DB_INGEST_PASSWORD", "ipw")
...
def test_role_falls_back_to_default_when_unset(monkeypatch):
    for k in list(__import__("os").environ):
        if k.startswith("SKYNET_CATALOG_DB_"):
            monkeypatch.delenv(k, raising=False)
    monkeypatch.setenv("SKYNET_CATALOG_DB_USER", "only_user")
    monkeypatch.setenv("SKYNET_CATALOG_DB_PASSWORD", "only_pw")
    s = CatalogSettings.from_env()
