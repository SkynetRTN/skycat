...
| [remote-catalogs.md](remote-catalogs.md) | **open** | Two-track coverage plan. Local first (L0–L4: size the candidates, then mirror Tycho-2, Gaia DR3 synthetic photometry, 2MASS, and hemisphere-driven deep optical); remote second (R0–R5), as an *adjacent* surface — `CatalogReader` stays local-only and a new `RemoteCatalogReader` owns VizieR and SIMBAD. Inventories the three legacy integrations (the pipeline plugin layer, the public-API SIMBAD resolver, the hand-rolled VizieR APASS route). Nothing implemented; L0's storage measurement and R0's decision record are both outstanding. |
