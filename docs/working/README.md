...
| [remote-catalogs.md](remote-catalogs.md) | **open** | Design and phased plan for remote catalog access as an *adjacent* surface: `CatalogReader` stays local-only, a new `RemoteCatalogReader` owns VizieR and SIMBAD. Inventories the three legacy integrations (the pipeline plugin layer, the public-API SIMBAD resolver, the hand-rolled VizieR APASS route). Nothing implemented; phase 0 is a decision record that has not been written. |
