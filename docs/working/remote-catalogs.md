...
being asked deliberately. Phase 0 (§6) writes that argument down as decision
...
| Lookup by native id | ✅ `lookup()` | ⏸ deferred | Expressible as a VizieR `column_filter`, but the id column differs per catalog. Phase 4 at the earliest |
...
| Parity verification | — | dev-only harness | Phase 1. The highest-value, lowest-risk piece, and it ships nothing |
...
the two backends agree — which is the claim Phase 1's parity harness exists to
...
Phase 1 (a parity harness that knows what the answer should be) preceding Phase 3
(shipping the query).
...
no local schema to design. That argument belongs in decision 0002 (Phase 0), not
...
`skycat/remote/mapping.py`, and a Phase 1 test that asserts it agrees with
...
Phase 3 should answer with "code first". `CUSTOM_VIZIER_CATALOGS` is the
...
plan and it belongs in Phase 5.
...
Six phases. Each is independently valuable and independently abandonable; each
names what it touches, what must be true before it is done, and the condition
under which it should stop rather than continue. Phases 0 and 1 together are the
majority of the risk reduction and about a week of work.

```
Phase 0  decision record ─────────┐
                                  ├──▶ Phase 2  reader + resolve() ──▶ Phase 3  first cone ──▶ Phase 4  more catalogs
Phase 1  mapping + parity (dev) ──┘                                          │
                                                                             └──▶ Phase 5  lint + consumer migration
```

### Phase 0 — Decide and record (0.5–1 day)
...
### Phase 1 — Mapping layer and parity harness, dev-only (3–5 days)
...
### Phase 2 — `RemoteCatalogReader` skeleton and `resolve()` (3–5 days)
...
### Phase 3 — First remote catalog: Tycho-2 (5–8 days)

**Scope.** `cone()` for exactly one catalog with **no local counterpart**, so
§5.2 cannot bite: Tycho-2 (`I/259/tyc2`, 2.5M rows, two bands, no transformation
expressions). Plus `skycat/remote/vizier.py` (transport, timeouts, bounded
retries), `catalogs()`, `describe()`, `mag_min`/`mag_max` rendered by the mapping
layer (§4.6), and a CLI `skycat remote cone` behind the extra.
...
**Stop here if** no consumer has asked by the time Phase 2 lands. Phases 0–2 are
valuable standalone; Phase 3 onward is coverage nobody has requested, and
`skynet-db` already has working remote providers for all six candidates.

### Phase 4 — Remaining remote-only catalogs (1–2 days each)

PanSTARRS (`II/349`), 2MASS (`II/246`), UCAC5 (`I/340`), USNO-B1.0 (`I/284`),
SkyMapper (`II/358/smss`) — added one def at a time, each with fixture tests, in
priority order set by whoever asked. Photometric transformations stay out
(§1.3.4). A general remote `QualityFilter` with `RemoteConstraintError` belongs
here if it is needed. `lookup()` by native id, if it is wanted, lands here too.
...
### Phase 5 — Designation lint and consumer migration (2–4 days)
...
in the stable docs exists. Extend it to `RemoteCatalogReader` when Phase 2 lands,
...
1. **Branch state of the downstream routing layer.** `catalogs/local/` in
   ...
   (excluding Landolt and Stetson). Blocks Phase 0's claim that routing is
   already downstream.
2. ~~**APASS DR10 on VizieR.**~~ **Verified 2026-08-07: still not queryable.**
   ...
   before Phase 4; the AAVSO page tracking the VizieR schedule is the source.
3. **Landolt 2009 designation.** `J/AJ/137/4186` is inferred from
   ...
   Confirm before adding a second `comparable="yes"` pair in Phase 1.
...
5. **VizieR TAP batch semantics.** Whether `TAPVizieR` via `pyvo` offers upload
   joins good enough to revisit §4.10. Measure in Phase 3; do not assume.
...
8. **Does anything besides the two public-api routes call SIMBAD?** §2.6 found one
   integration by grep. Confirm before Phase 5 plans its retirement.

---

## 9. What would change the plan

**Would justify accelerating Phase 3–4:**

- A consumer naming PanSTARRS or 2MASS photometry it needs with no plan to
  mirror. PanSTARRS is 1.9 billion rows; a local mirror is a serious
  infrastructure commitment and for occasional use it is unjustifiable. This
  remains the strongest argument in the document.
